<?php
	$data_relation = "ticketsIniciados";
	...
	if (isset($_POST['btn_edit_data_of_table'])){
		$query = "CALL `sp_waitingqueuebybarber_update_from_initialized_to_finalize`(";
	...
	function fnTraerDatos(){
		...
		$query = "CALL `sp_waitingqueuebybarber_show_employee_user_where_state`(2, " . $tabla[2][0] . " );";
	...
	function fnTraerDatosWhere($where){
		$tabla = fnTraerCodigoUsusario();
		$query = "CALL `sp_waitingqueuebybarber_show_employee_user_where_state_another`('%" . $where . "%', 2, " . $tabla[2][0] . ");";
