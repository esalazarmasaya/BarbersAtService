<?php
	...
	if (isset($_SESSION['trans_header_to_edit']) && !empty($_SESSION['trans_header_to_edit'])){
		...
		if (isset($_POST['sbmt_prove_transaction'])){
			$query = "CALL `sp_Payment_add_by_Transaction`(";
	...
	function fnDrawTransactionHeader(){
		...
		if (isset($_SESSION["trans_header_table"]) && !empty($_SESSION["trans_header_table"])){
			...
			$value = $value . '
					<div class="form-group">
						<button type="submit" class="btn btn-default" name="sbmt_prove_transaction">Comprobar</button>
					</div>
			';
